    ...
    extends PlanarGraph
{
   ...
   * @return
   */
  private Collection removeDangles()
  {
    List nodesToRemove = findNodesOfDegree(1);
    Set dangleLines = new HashSet();

    int i = 0;
    while (i < nodesToRemove.size()) {
      Node node = (Node) nodesToRemove.get(i++);

      List nodeOutEdges = node.getOutEdges().getEdges();
      // this edge might have already been removed
      if (nodeOutEdges.size() < 1) continue;
      DirectedEdge de = (DirectedEdge) nodeOutEdges.get(0);
      PolyEdge e = (PolyEdge) de.getEdge();
      dangleLines.add(e.getLine());

      Node toNode = de.getToNode();
      remove(node);
      // add the toNode to the list to be processed, if it is still in the graph
      if (findNode(toNode.getCoordinate()) != null && toNode.getDegree() == 1)
